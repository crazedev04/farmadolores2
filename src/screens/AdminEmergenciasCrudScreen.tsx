...
const AdminEmergenciasCrudScreen: React.FC = () => {
  ...
  const { colors } = theme;
  const [items, setItems] = useState<EmergenciaItem[]>([]);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      query(collection(db, 'emergencias'), orderBy('name')),
        (snapshot) => {
  ...
  const handleSelect = (item: EmergenciaItem) => {
  ...
  const handleSubmit = async () => {
    if (!name.trim()) {
      Alert.alert('Falta nombre', 'Ingresa el nombre de la emergencia.');
      return;
    }
    setSaving(true);
    try {
      ...
      if (editingId) {
        await updateDoc(doc(db, 'emergencias', editingId), payload);
      } else {
        await addDoc(collection(db, 'emergencias'), payload);
      }
      resetForm();
    } catch {
      Alert.alert('Error', editingId ? 'No se pudo actualizar la emergencia.' : 'No se pudo agregar la emergencia.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUploadImage = async () => {
    ...
    try {
      const result = await pickAndUploadImage('emergencias/main', {
        maxWidth: 1280,
        maxHeight: 1280,
        quality: 80,
      });
  ...
  const handleUploadGallery = async () => {
    ...
    try {
      const result = await pickAndUploadImage('emergencias/gallery', {
        maxWidth: 1280,
        maxHeight: 1280,
        quality: 75,
      });
  ...
  const handleDelete = (item: EmergenciaItem) => {
    Alert.alert('Eliminar emergencia', `Eliminar "${item.name}"?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await deleteDoc(doc(db, 'emergencias', item.id));
          } catch {
            Alert.alert('Error', 'No se pudo eliminar la emergencia.');
          }
        },
      },
    ]);
  ...
  const renderHeader = () => (
    <View>
      <Text style={[styles.title, { color: colors.text }]}>Gestion de emergencias</Text>

      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}
      >
        <View style={styles.formHeader}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>
            {editingId ? 'Editar emergencia' : 'Agregar emergencia'}
          </Text>
          {editingId && (
            <TouchableOpacity
        ...
        >
          {saving ? (
            ...
            >
              {editingId ? 'Guardar cambios' : 'Agregar emergencia'}
            </Text>
          )}
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        >
          Tocá una emergencia para editarla.
        </Text>
      </View>
