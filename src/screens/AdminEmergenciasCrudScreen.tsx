...
const AdminEmergenciasCrudScreen: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        await firestore().collection('emergencias').doc(editingId).update(payload);
      } else {
        await firestore().collection('emergencias').add(payload);
      }
      resetForm();
    } catch {
  ...
  const handleDelete = (item: EmergenciaItem) => {
    Alert.alert('Eliminar emergencia', `Eliminar "${item.name}"?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await firestore().collection('emergencias').doc(item.id).delete();
          } catch {
