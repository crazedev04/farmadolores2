...
import { farmaciaCrudSchema } from '../admin/validators';
...
const AdminFarmaciasScreen: React.FC = () => {
  const mapDoc = useCallback((id: string, data: Record<string, unknown>): FarmaciaItem => {
  ...
  return (
    <AdminEntityCrudScreen<FarmaciaItem>
      screenTitle="Gestion de farmacias"
      singularLabel="farmacia"
      searchPlaceholder="Buscar farmacia"
      emptyText="No hay farmacias cargadas."
      ...
      imageUploadPath="farmacias/main"
      galleryUploadPath="farmacias/gallery"
      mapDoc={mapDoc}
      mapItemToForm={mapItemToForm}
      validate={(form) => {
        const parsed = farmaciaCrudSchema.safeParse(form);
